...
/* ── HERO ENTRANCE ───────────────────────────────────────────────── */
/*
  Плашка → заголовок → подзаголовок → кнопки появляются поочерёдно.
  ✅ Пауза между элементами: измените 140 (миллисекунды).
*/
(function initHero() {
  ['.hero-kicker', 'h1', '.hero-lead', '.hero-btns'].forEach(function(sel, i) {
    var el = document.querySelector(sel);
    if (!el) return;
    setTimeout(function() { el.classList.add('in'); }, 180 + i * 140);
  });
})();
...
/* ── PARALLAX числа "26" ─────────────────────────────────────────── */
/*
  ✅ Скорость: измените 0.12 (0 = нет движения, 0.5 = быстро).
  На мобильных отключается через CSS.
*/
(function initParallax() {
  var yr = document.querySelector('.hero-yr');
  if (!yr) return;
  if (window.matchMedia('(max-width: 1100px)').matches) return;
  window.addEventListener('scroll', function() {
    yr.style.transform = 'translateY(' + (window.scrollY * 0.12) + 'px)';
  }, { passive: true });
})();
