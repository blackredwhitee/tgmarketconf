...
/* ── NAV STATE ───────────────────────────────────────────────────── */
/*
  При scrollY = 0 (hero): синий nav.
  При scrollY > 12: светлый nav с backdrop-blur.
  update() вызывается сразу при загрузке — корректно работает
  при открытии через якорную ссылку (например, #tickets).
*/
(function initNavShadow() {
  ...
  update(); /* проверить состояние сразу при загрузке */
})();
